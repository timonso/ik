...
#include "AAPosableCharacter.h"

// Sets default values
AAAPosableCharacter::AAAPosableCharacter()
{
	...
	static ConstructorHelpers::FObjectFinder<USkeletalMesh> MannequinMesh(TEXT("/Game/Mannequin/Character/Mesh/SK_Mannequin"));
	if (MannequinMesh.Succeeded())
	{
		...
		posableMeshComponent_reference->SetSkeletalMesh(default_skeletalMesh_reference);
	}
	else
	{
...
bool AAAPosableCharacter::initializePosableMesh()
{
...
void AAAPosableCharacter::waving_playStop()
{
	session1_isPlaying = !session1_isPlaying;
}

bool AAAPosableCharacter::doesBoneOrSocketNameExists(FName inputName)
{
	...
	return(
		...
		|| 
		posableMeshComponent_reference->DoesSocketExist(inputName)
	);
}

void AAAPosableCharacter::setVisibility(bool visible)
{
	// initialization check to avoid crashes.
	if (!posableMeshComponent_reference)
	{
...
void AAAPosableCharacter::storeCurrentPoseRotations(TArray<FRotator>& storedPose)
{
	// initialization check to avoid crashes.
	if (!posableMeshComponent_reference)
	{
...
void AAAPosableCharacter::waving_initializeStartingPose()
{
	// initialization check to avoid crashes.
	if (!posableMeshComponent_reference)
	{
...
void AAAPosableCharacter::waving_tickAnimation()
{
	// initialization check to avoid crashes.
	if (!posableMeshComponent_reference)
	{
...
void AAAPosableCharacter::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void AAAPosableCharacter::Tick(float DeltaTime)
{
